...
resource "aws_iam_role" "mimir_bucket_role" {
  name = "${var.pjname}-mimir-bucket-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${var.oidc}:sub" = "system:serviceaccount:monitoring:mimir-sa",
...
resource "aws_iam_role_policy_attachment" "mimir_bucket_policy_attachment" {
  role       = aws_iam_role.mimir_bucket_role.name
